...
const FolderForm = withFormik({
    mapPropsToValues: (props) => {
        console.log("map", props, props.folder)
        return {
    ...
    handleSubmit: (values, { props, setSubmitting, setErrors, resetForm }) => {
        console.log("hs val", values)
        console.log("hs pr", props)
        if (values._id !== -1 && values._id !== 0) {
            console.log("inside folder edit", values)
            ...
            console.log("inside folder add", values)
...
const mapStateToProps = (state, ownProps) => {
    ...
    console.log("go in?", ownProps.folder)
    if (ownProps.folder !== -1 && ownProps.folder !== 0) {
        console.log("inside")
        state.folders.filter(folder => {
            console.log(folder)
            console.log(folder._id)
            console.log(folder._id === ownProps.folder)
            return false
        })
        folder = state.folders.filter(folder => folder._id === ownProps.folder)[0]
    }
    console.log(folder)
    return {
        folder: folder,
